	...
	for _, bc := range cases {
		...
		if bc.name == "kueue-was-podgroup-api-mismatch" {
			...
			for _, text := range []string{
				...
				`The scheduler remained unready, and the reflector reported "failed to list *v1beta1.PodGroup: the server could not find the requested resource", confirming the cluster never initialized.`,
			...
			for _, text := range []string{
				...
				`The reflector reported "failed to list *v1beta1.PodGroup: the service account was forbidden".`,
				`The reflector reported "failed to list *v1beta1.ConfigMap: the server could not find the requested resource".`,
				`The reflector reported it never failed to list *v1beta1.PodGroup: the server could not find the requested resource.`,
				`The reflector reported no evidence it failed to list *v1beta1.PodGroup: the server could not find the requested resource.`,
				`The reflector reported "failed to list *v1beta1.PodGroup: the server could not find the requested resource" was not present in this run.`,
				`A non-reflector reported "failed to list *v1beta1.PodGroup: the server could not find the requested resource".`,
				`The reflector reported "failed to list *v1beta1.PodGroup: the server could not find the requested resourceful endpoint".`,
...
func writeBenchmarkJSONL(t *testing.T, path string, bc benchCase, repetition int, tc *models.TestCase, outcome benchmarkOutcome, elapsed time.Duration, snapshot ai.AnalysisTraceFile, observations []benchmarkDraftObservation, selectedAttempt int, toolUsage benchmarkToolUsage, traceSummary benchmarkTraceSummary, providerRequestCap int, cacheGeneration string, critiquePolicy ai.CritiqueCachePolicy, cacheVerification benchmarkCacheVerification, identity benchmarkRunIdentity, evidenceCoverage benchmarkEvidenceCoverage, stageReport benchmarkEvidenceStageReport) {
	...
	result := benchmarkJSONLResult{
		...
		Trace: benchmarkJSONLTrace{
			...
			RequestUsageSamples: []benchmarkRequestUsageSample{},
		},
...
func TestWriteBenchmarkJSONLIsBlindedAndPrivate(t *testing.T) {
	...
	writeBenchmarkJSONL(t, path, bc, 2, tc, benchmarkOutcomeUsable, 3*time.Second, snapshot, observations, 1,
		...
		benchmarkTraceSummary{floorNudges: 1, floorNudgeReasons: []string{"gcs_bytes"}}, 15, "generation", ai.CritiqueCachePolicyHard, cacheVerification,
