...
func logGenerationFailure(id string, code ReasonCode, failure *AnalysisFixFailureView, err error) {
	...
	if failure != nil && failure.Category == AnalysisFixFailureNoReviewablePatch &&
		...
		if failure.OperatorSummary != "" {
			log.Printf("action request %s: generation completed without a reviewable patch (reason=%s category=%s): %s; operator_summary=%q", id, code, category, detail, redact.OperatorText(failure.OperatorSummary))
			return
		}
	...
	if failure != nil && failure.OperatorSummary != "" {
		log.Printf("action request %s: generation failed (reason=%s category=%s): %s; operator_summary=%q", id, code, category, detail, redact.OperatorText(failure.OperatorSummary))
		return
	}
