...
func TestAppendLedgerWritesPrivateBoundedState(t *testing.T) {
	...
	for i := 0; i < maxLedgerRecords+1; i++ {
		...
		if err := AppendLedger(path, record); err != nil {
			t.Fatal(err)
		}
	}
...
func TestLedgerContainsAttempt(t *testing.T) {
	...
	found, err := LedgerContainsAttempt(path, attempt)
	if err != nil || !found {
		t.Fatalf("found=%v error=%v", found, err)
	}
	found, err = LedgerContainsAttempt(path, hashString("other"))
	if err != nil || found {
		t.Fatalf("found=%v error=%v", found, err)
	}
}
...
func TestAppendLedgerPrunesExpiredRecords(t *testing.T) {
	...
	for _, item := range []struct {
		...
		if err := AppendLedger(path, ledgerRecord(item.id, item.when)); err != nil {
			t.Fatal(err)
		}
	}
