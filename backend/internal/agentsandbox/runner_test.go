...
func TestValidateSpec(t *testing.T) {
	valid := Spec{Purpose: "analysis", RequestEnv: "PROW_AI_ANALYSIS_REQUEST_B64", Request: []byte(`{"version":1}`), Timeout: time.Minute, OutputLimitBytes: 4096}
	...
	for _, mutate := range []func(*Spec){
		...
		func(s *Spec) { s.Purpose = " analysis" },
		func(s *Spec) { s.Purpose = "analysis-" },
		func(s *Spec) { s.RequestEnv = "bad-name" },
		func(s *Spec) { s.RequestEnv = "PROW_AI_ANALYSIS_REQUEST_B64 " },
