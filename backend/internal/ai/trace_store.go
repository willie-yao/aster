...
func traceInformation(trace AnalysisTrace) int {
	...
	for _, event := range trace.Events {
		...
		if event.UsageReported || event.InputTokens > 0 || event.CachedInputTokens > 0 || event.CacheWriteInputTokensReported || event.CacheWriteInputTokens > 0 || event.OutputTokens > 0 || event.ReasoningTokens > 0 || event.Bytes > 0 || event.FinishReason != "" {
			score++
		}
	}
	return score
