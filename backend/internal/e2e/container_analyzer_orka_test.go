...
func TestOrkaContainerAnalyzerKind(t *testing.T) {
	...
	t.Run("scripted-flatcar-result", func(t *testing.T) {
		...
		scoreBenchCase(t, bc, &tc, benchmarkContainerOutcome(&tc), 0, "Orka container", 0, toolUsage, summarizeBenchmarkTrace(traceSnapshot), nil, 0)
	...
	if liveEndpoint != "" || liveModel != "" {
		...
		t.Run("live-kimi-flatcar", func(t *testing.T) {
			...
			traceSnapshot := ai.AnalysisTraceFile{Traces: state.Traces}
			toolUsage := successfulBenchmarkToolUsage(traceSnapshot)
			scoreBenchCase(t, bc, &tc, benchmarkContainerOutcome(&tc), elapsed, "Orka container Kimi", 500_000, toolUsage, summarizeBenchmarkTrace(traceSnapshot), nil, 0)
...
sequence = [response(tool=("c1","read_artifact",{"path":"build-log.txt"})), response(tool=("c2","tail_artifact",{"path":"build-log.txt"})), response(tool=("c3","read_artifact",{"path":"artifacts/junit.e2e_suite.1.xml"})), response(content=analysis), response(content=json.dumps({"findings":[]}))]
