	...
	for di := range details {
		...
		for ri := range detail.Runs {
			...
			for ti := range run.TestCases {
				...
				if testCase.Status != "failed" || testCase.AISummary == nil || testCase.AIAnalysis == nil || testCase.AIAnalysis.Mode != ai.AgenticMode || !ai.AnalysisCitationsVerified(testCase.AIAnalysis) {
					continue
				}
