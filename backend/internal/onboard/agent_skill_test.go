...
func TestConsumerSetupAgentSkill(t *testing.T) {
	...
	for _, anchor := range []string{
		...
		"@v0.10.0-rc.2", "<engine-ref>", "-engine-ref <engine-ref>", "default for Pages setup", "supports the Kubernetes storage flags",
	...
	for _, forbidden := range []string{
		...
		"-engine-ref v0.10.0-rc.2", "recorded engine revision is the commit resolved from that exact ref",
