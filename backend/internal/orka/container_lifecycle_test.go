...
func TestPruneContainerAnalysisBundlesRemovesOnlyExpiredInactiveBundles(t *testing.T) {
	...
	wantDeleted := []string{"configmaps/legacy@rv-legacy", "configmaps/missing@rv-missing", "configmaps/old@rv-old"}
...
func TestPruneContainerAnalysisBundlesIsBounded(t *testing.T) {
	now := time.Now()
	items := make([]unstructured.Unstructured, 0, ContainerAnalysisBundlePruneLimit+20)
	for i := 0; i < ContainerAnalysisBundlePruneLimit+20; i++ {
		items = append(items, bundleObject(fmt.Sprintf("old-%03d", i), "", now.Add(-2*ContainerAnalysisBundleRetention-time.Duration(i)*time.Minute)))
	}
	client := &fakeContainerResourceClient{listed: items}
	deleted, err := PruneContainerAnalysisBundles(context.Background(), client, "orka-system", now)
	if err != nil {
		t.Fatal(err)
	}
