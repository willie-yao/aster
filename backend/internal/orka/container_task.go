...
func BuildContainerAnalysisResources(in ContainerAnalysisTaskSpec) (ContainerAnalysisResources, error) {
	...
	if len(in.NodeSelector) == 0 {
		...
		nodeSelector := make(map[string]any, len(in.NodeSelector))
		for key, value := range in.NodeSelector {
			nodeSelector[key] = value
		}
		execution["nodeSelector"] = nodeSelector
	}
	if len(in.Tolerations) > 0 {
		tolerations := make([]any, 0, len(in.Tolerations))
		for _, toleration := range in.Tolerations {
			tolerations = append(tolerations, maps.Clone(toleration))
		}
		execution["tolerations"] = tolerations
	}
	if len(in.Affinity) > 0 {
		execution["affinity"] = maps.Clone(in.Affinity)
	}
