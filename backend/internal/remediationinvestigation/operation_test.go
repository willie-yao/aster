...
func (m *blockingOperationModel) ToolLoop(_ context.Context, _, _ string, _ *tools.Registry, _ []string, _ *tools.Env, opts ai.ToolLoopOptions) (string, error) {
	...
	if opts.Observe != nil {
		...
		opts.Observe(ai.ToolLoopEvent{Name: "read_repo_file", Path: "controllers/reconcile.go", BytesFetched: 80})
	}
	m.started <- struct{}{}
	<-m.release
	m.active.Add(-1)
...
func TestOperationFailedRefreshPreservesPreviousVerifiedResult(t *testing.T) {
	model := &fakeModel{
		...
		toolEvents: []ai.ToolLoopEvent{{Name: "read_artifact", Path: "builds/1/log.txt", BytesFetched: 19}, {Name: "read_repo_file", Path: "controllers/reconcile.go", BytesFetched: 80}},
	}
