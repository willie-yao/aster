{{- define "aster-platform.name" -}}
{{- default "prow-ai-dashboard-platform" .Values.nameOverride | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{- define "aster-platform.fullname" -}}
{{- if .Values.fullnameOverride -}}
{{- .Values.fullnameOverride | trunc 63 | trimSuffix "-" -}}
{{- else -}}
{{- $name := default "prow-ai-dashboard-platform" .Values.nameOverride -}}
...
{{- define "aster-platform.selectorLabels" -}}
app.kubernetes.io/name: {{ include "aster-platform.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end -}}

{{- define "aster-platform.labels" -}}
helm.sh/chart: {{ printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" }}
{{ include "aster-platform.selectorLabels" . }}
app.kubernetes.io/version: {{ .Chart.AppVersion | quote }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
app.kubernetes.io/part-of: prow-ai-dashboard-platform
{{- end -}}

{{- define "aster-platform.gatewayName" -}}
{{- printf "%s-%s" (include "aster-platform.fullname" .) .Values.modelGateway.name | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{- define "aster-platform.bindingName" -}}
{{- printf "%s-prow-ai-dashboard-platform-binding" .Release.Name | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{- define "aster-platform.gatewayImage" -}}
{{- printf "%s@%s" .Values.modelGateway.image.repository .Values.modelGateway.image.digest -}}
{{- end -}}

{{- define "aster-platform.executionPolicyHash" -}}
{{- toJson (list .Values.execution.networkPolicy.allowedFQDNs .Values.modelGateway.enabled (ternary .Release.Namespace "" .Values.modelGateway.enabled) (ternary (include "aster-platform.name" .) "" .Values.modelGateway.enabled) (ternary .Release.Name "" .Values.modelGateway.enabled) (ternary (toString .Values.modelGateway.service.targetPort) "" .Values.modelGateway.enabled)) | sha256sum -}}
{{- end -}}

{{- define "aster-platform.gatewayPolicyHash" -}}
...
{{- define "aster-platform.validatePublicHost" -}}
{{- $label := index . 0 -}}
{{- $host := lower (index . 1) -}}
...
{{- define "aster-platform.validateFQDNs" -}}
{{- $label := index . 0 -}}
{{- $values := index . 1 -}}
{{- range $values -}}
  {{- $value := lower . -}}
...
{{- define "aster-platform.validate" -}}
...
{{- include "aster-platform.validateFQDNs" (list "execution.networkPolicy.allowedFQDNs" .Values.execution.networkPolicy.allowedFQDNs) -}}
{{- $bindingName := include "aster-platform.bindingName" . -}}
...
{{- if .Values.modelGateway.enabled -}}
  ...
  {{- include "aster-platform.validatePublicHost" (list "modelGateway.publicHost" $gateway.publicHost) -}}
  {{- include "aster-platform.validatePublicHost" (list "modelGateway.upstreamHost" $gateway.upstreamHost) -}}
