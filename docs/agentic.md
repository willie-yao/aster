...
Causal-group parents do not create combined issue or fix drafts. When every member build remains available, an authenticated cause-scoped chat can inspect those failed builds and the newest later completed run. The later run is comparison evidence, not cause membership. Cause chat derives the existing observation-only recovery streak from that group's builds, but a passing comparison does not prove a fix: the model must compare the representative test and triggering inputs.
...
Chat does not change File Issue eligibility. A maintainer may request a Fix investigation from a completed answer, including one without verified citations, tied to the cause's representative exact JUnit failure. Historical file links are optional source hints, not admission requirements. Missing evidence, transient severity, and unverified claims remain qualified while the coding agent investigates the repository. Preflight resolves the explicit tested branch and preserves the failure-revision ancestry check. Preview confirmation rechecks the selected context, pinned repository/base, canonical patch, and execution-result integrity before any write.
