...
At admission, the action captures an immutable bounded snapshot of the selected answer, its qualifications, originating test or cause, exact JUnit target, retained citations, repository, tested branch, and generation base. A selected answer may reuse citations validated by earlier turns, but later turns do not alter the snapshot. Only retained validated citations are carried as verified artifact evidence. The action rechecks current cause membership and content, the exact JUnit analysis, and source/base identity before accepting a generated result and before confirmation. A stale target requires a new preview.

Deleting or expiring chat does not revoke an admitted Fix or a Fix request that has already captured its selected answer. Deleting chat before capture prevents a new admission. Use explicit action cancellation to cancel a pending or ready Fix. Existing owner-bound requests can be retrieved without chat; repeated admission reconnects to an existing request instead of silently repinning it. Older private Fix records without a complete action-owned handoff require regeneration. Unknown write outcomes retain reconciliation-only recovery and cannot use an incomplete handoff for another write.
...
Confirmation is a separate authenticated POST bound to the owner and exact preview. It rechecks the analysis or pattern identity, the action-owned selected finding when applicable, pinned source/base, destination policy, canonical patch, ordered executor results, warning state, and GitHub deduplication marker. Exact-JUnit confirmation also rechecks the tested branch head. Any bound identity or patch drift fails closed. The dashboard does not rerun target commands during confirmation and never silently regenerates content.
