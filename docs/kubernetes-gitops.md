...
cloud or Kubernetes infrastructure, create runtime Secret values, or contact a cluster.
...
credential material. It requires no cluster credentials and performs no deployment.
...
2. Installs or enables a supported Flux distribution using the cluster
   operator's normal process.
3. Configures Flux to reconcile the reviewed repository branch and `./gitops`
   path.
4. Uses a read-only repository credential when the repository is private. The
   credential is not stored in the generated bundle.
...
AKS operators may use the supported `microsoft.flux` extension. See the official
...
Use an extension version that supports the stable APIs generated by Aster.

Flux manages the Kubernetes resources declared by the chart releases. It does
not provision clusters, nodes, storage, networking, DNS, certificates, identity
systems, Secret-management systems, or other cloud or Kubernetes infrastructure.
