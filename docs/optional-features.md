...
server and in-process analysis. They do not require a Fix PR coding-agent
runtime.
...
| Causal remediation investigation | Kubernetes with authentication | Explicit read-only implementation-target triage with safe status only; File Issue and Fix PR stay blocked | [Causal remediation investigation](server.md#causal-remediation-investigation-api) |
...
| Fix PR generation | Kubernetes with authenticated actions and consumer-installed Agent Sandbox | Experimental, highest-risk code-writing automation | [Experimental Fix PR generation](fix-prs.md) |
