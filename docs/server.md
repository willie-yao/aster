...
exact reviewed draft through the existing confirmation workflow. Confirmation is
idempotent for the preview retention window: retrying the same token after a lost
success response returns the original URL, while a concurrent confirmation
returns 409 until the first attempt finishes.
