...
| `required prompt draft was not produced` | `--require-prompt-draft` was set and agent or API prompt preparation safely fell back. | Read the safe stage and category, fix the OpenCode runtime, reviewed provider, or source access, then retry. |
| Prompt drafting times out on a slow provider | Agent execution or API source retrieval and structured extraction exceeded the onboarding-specific budget. | Retry with `--prompt-timeout 30m` or another reviewed value. The fetcher timeout and project `ai.timeout` do not change onboarding drafting. |
