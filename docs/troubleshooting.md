...
| Orka prompt authoring reports cleanup pending | A valid prompt was produced, but exact Task deletion could not yet be confirmed. | Keep the prompt draft, inspect the named Orka Task with operator credentials, and complete cleanup before another identical run. |
